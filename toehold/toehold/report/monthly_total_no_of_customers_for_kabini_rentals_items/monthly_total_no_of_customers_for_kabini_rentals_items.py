...
def get_data_and_chart(filters):
    ...
    query = """
        ...
        WHERE si.docstatus = 0
            AND si.custom_category = 'Kabini-Rentals'
    """
    ...
    for row in data:
        chart_data['labels'].append(f"{row.month_year} - {row.item}")
    ...
    formatted_data = [[d.month_year, d.category, d.item, d.no_of_customers] for d in data]
